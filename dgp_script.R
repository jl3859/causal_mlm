...
ig_dat_function <- function(classrooms, students){
  ...
  low_score <- sample(c(rep(0,round(.3*low_count,0)),rep(1,low_count - round(.3*low_count,0))), low_count) # higher probability (70%) of treatment if test score is <= 65 
  high_score <- sample(c(rep(0,round(.5*high_count,0)), rep(1, high_count -round(.5*high_count,0))), high_count) #equal probability of treatment if test score is > 65 
  Z_stud <- rep(NA,N)
